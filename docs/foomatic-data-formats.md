...
This document is the schema reference for every JSON artifact produced by the pipeline described in [foomatic-pipeline-architecture.md](./foomatic-pipeline-architecture.md). All artifacts live under `public/foomatic-db/` and are served as static files — none require a backend to read — except the two build-time intermediates, `feature-matrix.json` and `recommendations.json`, which are written to the git-ignored `cache/foomatic/` and are not part of the static export.
...
## `cache/foomatic/recommendations.json`
