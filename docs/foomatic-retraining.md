...
5. `vectorize.ts` — produces `cache/foomatic/feature-matrix.json` (skipped with `--skip-similarity`)
6. `compute-similarity.ts` — produces `cache/foomatic/recommendations.json` and `public/foomatic-db/recommendations/<id>.json` (skipped with `--skip-similarity`)
...
- **Every PR / push to `master`:** `build.yml` runs the full pipeline, including similarity computation, so a PR that breaks any stage fails its checks before merge. Nothing is deployed from this workflow.
...
Repeat the same pattern for `cache/foomatic/feature-matrix.json` (after `yarn foomatic:data:vectorize`) and `cache/foomatic/recommendations.json` (after `yarn foomatic:data:similarity`). This is how the `lib/foomatic/*` extraction refactor (see the test suite under `lib/foomatic/__tests__/`) was verified to be behavior-preserving before being merged.
